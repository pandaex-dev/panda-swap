    ...
    LPTokenWrapper, Ownable
{
    ...
    uint256[] public _amountList;
    mapping(address => uint256) public _index;
    mapping(address => uint256) public _unlockTimestamp;
    ...
    function stake(uint256 amount)
        ...
        checkStart
    {
        require(amount > 0, 'PNDLockSharePool: Cannot stake 0');
        super.stake(amount);
        if(_unlockTimestamp[msg.sender] == 0){
            _index[msg.sender] = _accountList.length;
            _accountList.push(msg.sender);
            _amountList.push(balanceOf(msg.sender));
        }else{
            _amountList[_index[msg.sender]] = balanceOf(msg.sender);
        }
        _unlockTimestamp[msg.sender] = block.timestamp + _lockDuration;
    ...
    function withdraw(uint256 amount)
        ...
        checkStart
    {
        require(amount > 0, 'PNDLockSharePool: Cannot withdraw 0');
        require(block.timestamp > _unlockTimestamp[msg.sender] || block.timestamp > periodFinish, 'Not the time');
    ...
    function getAmountList() public view returns (uint256[] memory) {
    	return _amountList;
    }
}
